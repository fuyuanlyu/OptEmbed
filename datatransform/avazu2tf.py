...
class AvazuTransform(DataTransform):
    def __init__(self, dataset_path, path, min_threshold, label_index, ratio, store_stat = False, seed = 2021):
       super(AvazuTransform, self).__init__(dataset_path, store_stat=store_stat, seed=seed )
       self.threshold = min_threshold
       self.label = label_index
       self.split = ratio
       self.path = path
       #self.data = self._read(name = None, header = 0, sep = ",", label_index = self.label)
...
if __name__ == "__main__":
    tranformer = AvazuTransform(args.dataset, args.record, 
